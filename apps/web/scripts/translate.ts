 ...
 *
 * Translates German source translation files to all supported locales.
...
const SOURCE_LOCALE = 'en'; // English is the source language
const TARGET_LOCALES = ['de', 'es']; // Languages to translate to
const NAMESPACES = [
  'common',
  'auth',
  'account',
...
async function translateBatch(
  ...
  const systemPrompt = `You are a skilled translator. Translate the following JSON object from English to ${targetLanguage}.

ABOUT THE PRODUCT:
ZeitPal is a modern HR software for leave and absence management, designed for companies. It helps teams:
- Manage employee vacation requests and approvals
- Track sick leave and other absences
- Plan team calendars and avoid scheduling conflicts
- Stay compliant with labor laws

The product is professional yet friendly, designed to make HR tasks simple and efficient.

TONE & STYLE:
- Use professional but friendly language
- Be clear and concise
- Use appropriate formality for business software
- For German: use formal "Sie" form, professional business German
- For Spanish: use standard neutral Spanish (suitable for all Spanish-speaking regions)
...
3. Preserve HTML tags if present (e.g., <strong>, <br/>, <terms>, <privacy>)
4. Maintain the same JSON structure and keys - only translate the values
5. Keep brand names (ZeitPal, etc.) untranslated
6. Keep technical terms that are commonly used in English (API, etc.)`;

  const userPrompt = `Translate this JSON from English to ${targetLanguage}:
  ...
  try {
    ...
    for (const key of keys) {
      if (!(key in translated)) {
        ...
        translated[key] = strings[key]!; // Fallback to German
      }
...
async function translateNamespace(
  ...
  if (fs.existsSync(targetFile)) {
    try {
      ...
      console.warn(
        `  Could not parse existing translations for ${targetLocale}/${namespace}`,
      );
    }
  }
  ...
  if (dryRun) {
    console.log(
      `  [DRY RUN] Would translate ${Object.keys(keysToTranslate).length} keys`,
    );
    return stats;
  }
...
async function translateLocale(
  ...
  for (const namespace of namespaces) {
    ...
    const stat = await translateNamespace(anthropic, namespace, locale, dryRun);
...
async function main() {
  ...
  console.log('==================');
  console.log(`Source: ${SOURCE_LOCALE} (English)`);
  console.log(`Targets: ${localesToProcess.join(', ')}`);
  console.log(`Namespaces: ${namespacesToProcess.join(', ')}`);
  console.log(
    `Mode: ${parallel ? 'PARALLEL' : 'Sequential'}${dryRun ? ' (DRY RUN)' : ''}`,
  );
  console.log('');
  ...
  if (parallel) {
    ...
    console.log(
      `Starting parallel translation of ${localesToProcess.length} locales...`,
    );
    ...
    for (const locale of localesToProcess) {
      const stats = await translateLocale(
        anthropic,
        locale,
        namespacesToProcess,
        dryRun,
